...
const UpperToolBar: React.FC<UpperToolBarProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // 1. Check for Alt key (and ensure no other modifiers like Ctrl/Cmd are pressed)
      if (e.altKey && !e.ctrlKey && !e.metaKey) {
        // 2. Use e.code to target the digit keys specifically (Digit0 - Digit9)
        const match = e.code.match(/^Digit(\d)$/);

        if (match) {
          const digit = parseInt(match[1]); // Extract "1" from "Digit1"
          const pageIndex = digit - 1;

          // 3. Bounds check based on your tool pages
          if (pageIndex >= 0 && pageIndex < TOPTOOLS.length) {
            e.preventDefault(); // Stop the character from typing in the editor
            ...
            setShowTableDialog?.(false); // Close submenus if they exist

            // 5. Shift Focus to the first button of the dock
            setTimeout(() => {
    ...
    // Using 'true' (Capture Phase) ensures your shortcut beats the text editor
    window.addEventListener("keydown", handleKeyDown, true);
    return () => window.removeEventListener("keydown", handleKeyDown, true);
  }, [setTopPage, TOPTOOLS.length, setShowTool, setShowColorPicker]);
  ...
  // 1. Fixed the Type to use Browser-compatible 'number'
  const idleTimerRef = useRef<number | null>(null);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // 1. Check for Alt key only
      if (e.altKey && !e.ctrlKey && !e.metaKey) {
        // OPTION A: Toggle Shortcut (Alt + H)
        if (e.code === "KeyH") {
          e.preventDefault();
          setShowTool((prev) => !prev);
          // Clear the 30s timer if it exists so it doesn't auto-close later
        ...
        // OPTION B: Your existing Alt + Digit logic
        const match = e.code.match(/^Digit(\d)$/);
        if (match) {
          ...
          if (pageIndex >= 0 && pageIndex < TOPTOOLS.length) {
            e.preventDefault();
            setTopPage(pageIndex);
            setShowTool(true); // Force show when switching pages
            setShowColorPicker(false);
  ...
  return (
    <div className="relative">
      ...
      >
        <AnimatePresence>
          {showTool && (
            <motion.div
              ref={toolbarRef}
              className="relative flex items-center justify-center px-6 p-0.5 h-12 w-full"
              initial={{ height: 0 }}
              animate={{ height: "3rem" }}
              exit={{ height: 0 }}
      ...
      <div
        ...
        style={{ left: `48rem`, transform: "translateX(-50%)" }}
      >
        <AnimatePresence>
          {openLinkDialog && (
            <motion.div
      ...
      <div className="absolute right-152 top-12">
        <AnimatePresence>
          {showColorPicker && (
            <motion.div
              initial={{ opacity: 0, y: -5 }}
      ...
      {/* Table Dialog */}
      <div className="absolute right-172 top-12">
        <AnimatePresence>
          {showTableDialog && (
            <motion.div
              initial={{ opacity: 0, y: -5 }}
